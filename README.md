...
- `DB_ENCRYPTION_KEY` — ключ шифрования данных перед записью в файл или PostgreSQL. Используйте стабильное значение, иначе старые данные нельзя будет расшифровать. Если ключ уже сменился, сервер сохранит нечитаемый старый снимок в `app_state` с ключом вида `default:unreadable:<timestamp>` и начнёт с пустой базы.
